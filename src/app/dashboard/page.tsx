...
export default function DashboardPage() {
  const immediate = previewClients.filter((client) => client.status === 'Comprar agora');
  return (
    <div className="space-y-7">
      ...
      <section className="metric-strip grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ['Clientes ativos', previewClients.length, <Building2 key="clients" size={18} />],
          ['Comprar agora', immediate.length, <CalendarClock key="cycle" size={18} />],
          ['Cotações no mês', 12, <ReceiptText key="quotes" size={18} />],
          ['Potencial da carteira', money(previewClients.reduce((sum, item) => sum + item.total12m, 0)), <TrendingUp key="potential" size={18} />],

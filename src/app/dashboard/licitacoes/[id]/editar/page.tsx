...
import { formatAppDate, formatDateInputValue, toIsoDate } from '../../../../../lib/date';
...
export default function EditarLicitacaoPage() {
  ...
  const [existingLicitacao, setExistingLicitacao] = useState<Licitacao | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [catalogData, licitacaoData] = await Promise.all([
          db.getProductCatalog(),
          db.getLicitacao(licitacaoId),
        ]);
        setCatalog(catalogData);

        if (!licitacaoData) {
          setErrorMsg('Licitacao nao encontrada.');
          return;
        }

        setExistingLicitacao(licitacaoData);
        setAno(licitacaoData.ano);
        setOrgao(licitacaoData.orgao || '');
        setNumeroPregao(licitacaoData.numero_pregao || '');
        setNumeroProcesso(licitacaoData.numero_processo || '');
        setModalidade(licitacaoData.modalidade || '');
        setDataAbertura(formatAppDate(licitacaoData.data_abertura));
        setDataVencimento(formatAppDate(licitacaoData.data_vencimento));
        setCodigoCliente(licitacaoData.codigo_cliente || '');
        setCarteiraRegiao(licitacaoData.carteira_regiao || '');
        setCidade(licitacaoData.cidade || '');
        setEstado(licitacaoData.estado || '');
        setOrgaoEmail(licitacaoData.orgao_email || '');
        setOrgaoTelefone(licitacaoData.orgao_telefone || '');
        setOrgaoContato(licitacaoData.orgao_contato || '');
        setStatus(licitacaoData.status);
        setObservacoes(licitacaoData.observacoes || '');
        setItems((licitacaoData.itens || []).map((item) => ({
          id: item.id,
          ...
          preco_minimo: item.preco_minimo == null ? '' : formatDecimalText(item.preco_minimo),
          valor_unitario: item.valor_unitario ? formatDecimalText(item.valor_unitario) : '',
          status: item.status,
          observacoes: item.observacoes || '',
        })));
      } catch (err) {
        console.error(err);
        setErrorMsg('Erro ao carregar a licitacao para edicao.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newLicitacao: Omit<Licitacao, 'created_at' | 'updated_at'> = {
        ...(existingLicitacao as Licitacao),
        id: licitacaoId,
        ano,
        orgao: orgao.trim(),
  ...
  if (initialLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-sm text-slate-400">Carregando licitacao para edicao...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex items-center gap-4">
        <Link
          href={`/dashboard/licitacoes/${licitacaoId}`}
        ...
        <div>
          <h1 className="text-3xl font-semibold text-slate-50">Editar Licitação</h1>
          <p className="text-slate-400 text-sm mt-1">Atualize dados, itens, resultado, quantidade ganha e valor homologado.</p>
        </div>
      </div>
